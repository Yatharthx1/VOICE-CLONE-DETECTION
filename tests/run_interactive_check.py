...
from train_detector import generate_synthetic_audio
...
def print_master_dashboard(output: VerificationOutput, title: str = "VOICE INTEGRITY VERIFICATION REPORT", debug: bool = False):
    ...
    # 1. Verdict
    print(f"\n🏆 [1. FINAL AUTHENTICITY VERDICT & PROBABILITY CALIBRATION]")
    verdict_badge = f"[{ass.verdict.value}]"
    risk_badge = f"[{ass.risk_level.value}]"
    print(f"  * Final Verdict        : {verdict_badge} — {ass.verdict_category}")
    print(f"  * Calibrated AI Prob   : {ass.ai_probability * 100:.1f}%")
    print(f"  * Evidence Confidence  : {ass.confidence * 100:.1f}%")
    print(f"  * Window Consistency   : {output.window_consistency * 100:.1f}%")
    print(f"  * Raw Model Score      : {ass.raw_score:.3f}")
    ...
    print(f"  * Verdict Explanation  : {ass.verdict_explanation}")
    ...
    # 4. Multi-Window Temporal Predictions
    if output.window_predictions and len(output.window_predictions) > 1:
        print(f"\n⏱️ [4. MULTI-WINDOW TEMPORAL CONSISTENCY]")
        win_str = ", ".join([f"W{i+1}: {p * 100:.1f}%" for i, p in enumerate(output.window_predictions)])
        print(f"  * Window Breakdown     : [{win_str}]")
        print(f"  * Window Consistency   : {output.window_consistency * 100:.1f}% ({'High Consistency' if output.window_consistency >= 0.75 else 'Moderate / Variable'})")

    # 5. Feature Branches
    print(f"\n🔬 [5. PARALLEL FEATURE EXTRACTION MODULES]")
    print(f"  ┌─────────────────────────┬──────────────┬─────────────────────────────────────────────────┐")
    ...
    # 6. ML & Biometrics
    print(f"\n🤖 [6. ML DEEPFAKE DETECTOR & SPEAKER BIOMETRICS]")
    print(f"  * Checkpoint Status    : {'[ACTIVE] TRAINED CHECKPOINT LOADED' if ml.is_checkpoint_loaded else '[BASELINE] DEFAULT DISTRIBUTIONS'}")
    print(f"  * ML Deepfake Score    : {ml.synthetic_probability * 100:.1f}% ({'SYNTHETIC CLASSIFICATION' if ml.is_synthetic else 'GENUINE CLASSIFICATION'})")
    ...
    # 7. Risk Drivers
    print(f"\n🚨 [7. FORENSIC RISK DRIVERS & ANOMALIES]")
    if ass.risk_factors:
    ...
    if output.alert:
        print(f"\n🔔 [8. AUTOMATED INTERVENTION DISPATCH]")
        print(f"  * Alert Dispatched     : ID #{output.alert.alert_id[:8]} -> Channels: {', '.join(output.alert.target_channels)}")

    if debug and output.debug_info:
        print(f"\n🛠️ [DEBUG TELEMETRY]")
        for k, v in output.debug_info.items():
            print(f"  * {k:25s}: {v}")
...
def generate_sample_suite(output_dir: Path) -> list[Path]:
    ...
    # 1. Genuine Human Speech Simulation (Natural Biological Dynamics)
    h_path = output_dir / "sample_genuine_human.wav"
    h_audio = generate_synthetic_audio(is_ai=False, duration_sec=4.5, seed_offset=100)
    sf.write(str(h_path), h_audio, 16000, subtype="PCM_16")
    samples.append(h_path)

    # 2. Synthetic AI Voice Simulation (HiFi-GAN Vocoder Artifacts)
    ai_path = output_dir / "sample_synthetic_ai_vocoder.wav"
    ai_audio = generate_synthetic_audio(is_ai=True, duration_sec=4.5, seed_offset=201)
    sf.write(str(ai_path), ai_audio, 16000, subtype="PCM_16")
    samples.append(ai_path)

    # 3. 48kHz WAV speech call
...
def main():
    ...
    parser.add_argument("--debug", action="store_true", help="Print detailed debug telemetry")
    parser.add_argument("--checkpoint", type=str, default=None, help="Path to custom trained model checkpoint")
    ...
    engine = VoiceIntegrityEngine(
        default_scenario=RiskScenario(args.scenario),
    ...
    if args.input:
        ...
        print_master_dashboard(output, title=f"VERIFICATION REPORT: {input_path.name}", debug=args.debug)
        ...
        for sample_path in test_samples:
            ...
            print_master_dashboard(output, title=f"SAMPLE REPORT: {sample_path.name}", debug=args.debug)
