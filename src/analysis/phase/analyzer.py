...
class PhaseAnalyzer(BaseAnalysisModule):
    ...
    def _evaluate_phase_anomalies(
        ...
        # 1. Harmonic Phase Clustering Failure (Vocoder Phase Incoherence)
        w_clust = 0.60
        total_weights += w_clust
        if if_res.if_harmonic_clustering_score < 0.08 and dur_sec > 1.5:
            anomaly_points += w_clust * (1.0 - if_res.if_harmonic_clustering_score / 0.08)
        ...
        # 2. Phase Dispersion Entropy (Only in conjunction with high unwrapped phase roughness)
        w_entropy = 0.40
        total_weights += w_entropy
        if gd.phase_dispersion_entropy > 0.98 and gd.unwrapped_phase_roughness > 6.0:
