...
class SpectralAnalyzer(BaseAnalysisModule):
    ...
    def analyze(self, audio: np.ndarray, sample_rate: int = 16000) -> SpectralAnalysisResult:
        ...
        if audio is None or len(audio) == 0 or np.max(np.abs(audio)) < 0.015:
    ...
    def _evaluate_spectral_anomalies(
        ...
        w_hf = 0.40
        total_weights += w_hf
        if hf.has_artificial_cutoff and hf.cutoff_frequency_hz:
        ...
        # 2. Spectral Flatness Check (Extreme flatness > 0.65 indicates synthetic unvoiced vocoder hiss)
        w_flatness = 0.35
        total_weights += w_flatness
        if dynamics.flatness_mean > 0.65:
            dev = (dynamics.flatness_mean - 0.65) / 0.25
            anomaly_points += w_flatness * min(1.0, dev)
            indicators.append(f"Atypical spectral flatness ({dynamics.flatness_mean:.3f} > 0.65)")

        # 3. Spectral Flux Stability Check (Over-smoothed spectra in some diffusion models)
        w_flux = 0.25
        total_weights += w_flux
        if dynamics.flux_mean < 0.003 and dur_sec > 1.5:
