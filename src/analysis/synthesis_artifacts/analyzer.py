...
class SynthesisArtifactsAnalyzer(BaseAnalysisModule):
    ...
    def analyze(self, audio: np.ndarray, sample_rate: int = 16000) -> SynthesisArtifactsResult:
        """Run complete synthesis artifacts analysis on audio array."""
    ...
    def _evaluate_artifact_anomalies(
        ...
        # 1. Neural Vocoder 2D Periodic Pattern (Transposed Convolution Check)
        w_voc = 0.50
        total_weights += w_voc
        ...
        elif voc.checkerboard_energy_ratio > 0.12:
            anomaly_points += w_voc * min(1.0, (voc.checkerboard_energy_ratio - 0.12) / 0.08)

        # 2. Severe Splicing Discontinuities (Energy jump > 25dB with phase step)
        w_splice = 0.50
        total_weights += w_splice
        if concat.splice_points_detected >= 6 and concat.max_energy_jump_db > 24.0:
            anomaly_points += w_splice * min(1.0, (concat.splice_points_detected - 5) * 0.25)
