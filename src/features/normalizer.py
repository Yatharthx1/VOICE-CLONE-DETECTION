...
class FeatureNormalizer:
    ...
    def fit(self, feature_matrix: np.ndarray) -> "FeatureNormalizer":
        """Fit means and standard deviations from a 2D array of feature vectors (N x D)."""
        if feature_matrix.ndim == 1:
            feature_matrix = feature_matrix.reshape(1, -1)
        self.means = np.mean(feature_matrix, axis=0).astype(np.float32)
    ...
    def to_dict(self) -> dict:
        """Serialize normalizer parameters to dictionary."""
        return {
    ...
    def load(cls, filepath: str) -> "FeatureNormalizer":
        """Load normalizer parameters from a JSON file."""
        import json
