...
class StreamingAudioBuffer:
    """
    Ring buffer for real-time live streaming audio ingestion (e.g. VoIP / Telephony packets).
    Accumulates incoming live frames and yields ready analysis windows for continuous scoring.
    """
    ...
    def __init__(
        ...
        win_sec = window_duration_sec if window_duration_sec is not None else self.config.chunk_window_sec
        hop_sec = hop_duration_sec if hop_duration_sec is not None else self.config.chunk_hop_sec
        min_sp_sec = min_speech_duration_sec if min_speech_duration_sec is not None else self.config.min_chunk_duration_sec

        self.window_samples = int(win_sec * self.target_sr)
        self.hop_samples = int(hop_sec * self.target_sr)
        self.min_samples = int(min_sp_sec * self.target_sr)
