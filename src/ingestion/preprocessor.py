...
class AudioPreprocessor:
    ...
    def normalize(
        ...
        if rms < 0.015:
            return audio  # Ambient noise / silence: DO NOT amplify low-level background noise

        current_dbfs = 20.0 * np.log10(rms)
        # Limit max gain boost to +12 dB so noise is never artificially amplified
        gain_db = min(12.0, max(-30.0, target_db - current_dbfs))
